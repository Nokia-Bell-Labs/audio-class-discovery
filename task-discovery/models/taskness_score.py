...
from models.tasks import CIFARClassificationTask, AudiosetClassificationTask
import utils
    ...
    def __init__(
        ...
        **kwargs
        ...
        if arch == 'resnet18':
            ...
            model_f = lambda: ResNet8(in_dim=in_dim, out_dim=n_classes, width_factor=width_factor)
        elif arch == 'mlp':
            model_f = lambda: FCNet(out_dim=n_classes, batch_norm=True)
        elif arch == 'vit':
            model_f = lambda: ViT4(out_dim=n_classes, batch_norm=True)
        elif arch == 'vgg16':
            model_f = lambda: VGG16(in_dim = in_dim, num_classes=n_classes)
        elif arch == 'vgg19':
            model_f = lambda: VGG19(in_dim = in_dim, num_classes=n_classes)
...
class TasknessScore(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # TODO: add other benchmarks, such as esc50, ...
        print('=======>>>>>>', kwargs.get('random_labelling', False))
        print(f"!!! CONFIRM self.dataset !!! {self.dataset}")
        if 'audioset' in self.dataset:
            selective_mapping = {
                'audioset': 'original',
            ...
            self._data_module = AudiosetDataModule(
                ...
                n_classes=kwargs.get('K', num_real_n_classes_dict[selective_mapping[self.dataset]]),
                ...
                selective= selective_mapping[self.dataset]
            )
...
class TasknessScoreWithTaskNet(TasknessScore):
    ...
    def _add_current_task_to_pool(self):
        ...
        if 'audioset' in self.dataset:
            task = AudiosetClassificationTask(task_type='table')
        else:
    ...
    def _reg_step(self):
        ...
        for j in range(self.hparams.n_simsteps):
            ...
            tmp_sim = (new_pred == tasks_pred).float().mean(1)  # calculate element-wise equality between 'new_pred' and 'task_pred'
            similarity += tmp_sim
